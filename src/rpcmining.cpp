...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    if(nBestHeight > 3500200)
    {
        obj.push_back(Pair("stakeinterest",    (uint64_t)COIN_YEAR_REWARD1));
    }
    obj.push_back(Pair("stakeinterest",    (uint64_t)COIN_REWARD1));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "CreedTokens is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "CreedTokens is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "CreedTokens is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "CreedTokens is downloading blocks...");
