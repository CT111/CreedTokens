...
#if defined(NDEBUG)
# error "CreedTokens cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 3 * 60 * 60; // 15 Hours
...
const string strMessageMagic = "CreedTokens Signed Message:\n";
...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        ...
        // keys. (remCreedTokens the 520 byte limit on redeemScript size) That works
...
{
    int64_t nSubsidy = COIN_REWARD1; //
    if(nBestHeight > 3500200) // Forever
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD1 * 33 / (365 * 33 + 8); // 1%
    }
...
{
    RenameThread("CreedTokens-loadblk");
